...
import {BATTERY_ASSUMPTIONS, simulateDailyEnergy} from "../../src/components/graph/simulation";
...
describe("daily energy simulation", () => {
    it("does not dispatch storage when no battery is installed", () => {
        ...
        expect(result.every(({batteryChargeKwh, batteryDischargeKwh, batteryStateOfChargeKwh}) =>
    ...
    it("grid-charges only in the cheap window and stops at the 50% target", () => {
        ...
        expect(result.filter(({hour}) => hour >= 5 && hour < 16).every(({batteryChargeKwh}) =>
            batteryChargeKwh === 0,
        )).toBe(true);
    ...
    it("charges from surplus solar outside the cheap window", () => {
        ...
        expect(result.some(({hour, batteryChargeKwh}) => hour >= 8 && hour < 16 && batteryChargeKwh > 0)).toBe(true);
    });

    it("discharges only during the peak window", () => {
        const result = simulateDailyEnergy(scenario({battery}));

        expect(result.some(({hour, batteryDischargeKwh}) => hour >= 16 && hour < 19 && batteryDischargeKwh > 0)).toBe(true);
        expect(result.filter(({hour}) => hour < 16 || hour >= 19).every(({batteryDischargeKwh}) =>
    ...
    it("keeps state of charge within reserve and total capacity", () => {
        ...
        for (const hour of result) {
            ...
            expect(hour.batteryDischargeKwh).toBeLessThanOrEqual(BATTERY_ASSUMPTIONS.maxDischargeKwPerUnit);
        }
    });
    ...
    it("does not charge the battery during a peak solar surplus", () => {
        const peak = simulateDailyEnergy(scenario({
