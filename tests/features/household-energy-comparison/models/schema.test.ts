...
describe('form-to-domain mapping', () => {
  ...
  it('maps absent equipment into proposed assets', () => {
    const assessment = toHouseholdAssessment(
      draft({
        hasSolar: false,
        hasBatteries: false,
        solar: { panelCount: 8 },
  ...
  it('does not require hidden EV fields when there is no EV', () => {
    expect(() =>
      toHouseholdAssessment(draft({ hasElectricVehicle: false, electricVehicle: {} })),
    ).not.toThrow()
  })
})
...
describe('scenario composition', () => {
  ...
  it('does not create redundant proposals for installed equipment', () => {
    expect(buildHouseholdScenarios(toHouseholdAssessment(draft())).map(({ id }) => id)).toEqual([
      'current',
    ])
  })
})
